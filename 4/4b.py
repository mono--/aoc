phrases="""pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
#recursively generate a list of all anagrams of a word;

#a faster/easier solution would be to just sort the letters in a given word...
def anagrams(word,anag):
	if len(word)==0:
		return([anag]);
	ret=[];
	for i in range(len(word)):
		w=list(word);
		w.pop(i);
		ret.extend(anagrams(w,anag+word[i]));
	return ret;
...
#start assuming all phrases are valid
valid=len(rows);

for row in words:
	anags=[];
	for word in row:
		if word not in anags:
			anags.extend(anagrams(word,""))
		else:
			#not a valid phrase
			valid=valid-1;
			break; #skip the remaining words in the phrase
