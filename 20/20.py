...
poss=[];
for dat in data:
	poss.append(dat[0].split('=')[1]);
	vels.append(dat[1].split('=')[1]);
	accs.append(dat[2].split('=')[1]);
...
for a in accs:
	...
	a_vecs.append(a[1:len(a)-1].split(','));

for p in poss:

	p_vecs.append(p[1:len(p)-1].split(','));

for v in vels:

	v_vecs.append(v[1:len(v)-1].split(','));
...
for i in range(200):
	print "step", i, len(p_vecs)
	for p in p_vecs:
		try:
			coll=p_vecs[p_vecs.index(p)+1:].index(p);
			remd=rem(p_vecs,p);
			print "rem", remd;
			p_vecs=[p_vecs[i] for i in range(len(p_vecs)) if i not in remd];
			v_vecs=[v_vecs[i] for i in range(len(v_vecs)) if i not in remd];
			a_vecs=[a_vecs[i] for i in range(len(a_vecs)) if i not in remd];
		except ValueError:
			pass;
				

	for x in range(len(p_vecs)):
		v_vecs[x] = add(a_vecs[x], v_vecs[x]);
		p_vecs[x] = add(p_vecs[x], v_vecs[x]);
